...
export const createNewRegisterUser = async (register: IRegister): Promise<object> => {
...
export const searchRegister = async (email: string | undefined): Promise<object> => {
    if(email === undefined){
        const isRegister = await RegisterEntity.find({relations: {user: true}})
        return isRegister
    }else{
        const isUser = await UserEntity.find({where: {email}, relations:{register: true}})
        if(!isUser[0]){
            throw new Error('El usuario no existe, debe registarse!')
        }else{
            if(!isUser[0].register[0]){
                throw new Error("El usuario no tiene registros cargados por el momento!")
            }else{
